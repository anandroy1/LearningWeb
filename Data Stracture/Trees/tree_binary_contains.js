...
class BinarySearchTree {
    ...
    // add a value to the tree while maintaining the
    // sortedness of the tree so lesser values are always hung
    // left on a node and greater values are always hung right 
    // on a node.
    add(value) {
        if (this.root === null) {
    ...
    // Write a function so it takes advantage of the structure
    // of the Binary search Tree. This function should execute
    // in 0(log2 N) time. Do not simply write a function that
    // checks every node. Force the functio to check in the 
    //  direction that nodes should be. REturn false if you 
    // ever move in a direction and there is no node there
    // where you would except the value to be.
...
console.log("contains:", tree.contains(90))
